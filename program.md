...
Mouse controls are disabled for the main game window: keyboard controls start, jump, pause, and mute, while pointer input on the canvas should not start or jump. A separate beetle-gold UI sits outside the canvas. Small clickable beetle buttons periodically appear there; clicking one grants 1 persistent gold in `bugBlasterGold` and removes that beetle.
