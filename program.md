...
Crash-hardening checks: all Canvas image draws route through the asset manager so broken or not-yet-ready SVG assets cannot throw out of the render loop. The animation loop catches and logs unexpected frame errors, then schedules the next frame. When verifying Beetle changes, play or automate through first Beetle spawn, gasoline pool creation/ignition, recharge phase, second-bar defeat, fakeout, graveyard resume, and any deferred level-up lootbox.
